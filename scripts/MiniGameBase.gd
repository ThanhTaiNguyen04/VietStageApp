...
func _show_mode_selection_menu() -> void:
	...
	$Root/TopBar/TopM/TopH/Title.text = "TRÒ CHƠI NHỎ - " + get_instrument_title()
...
func _start_note_round() -> void:
	...
	prompt_label.text = "Lắng nghe âm sắc %s và nhận diện xem đó là nốt nhạc dân tộc nào:" % get_note_instrument_label()
...
func _start_rhythm_round() -> void:
	...
	drum_btn.text = get_rhythm_btn_text()
	...
	prompt_label.text = "%s đúng nhịp khi vạch đỏ quét qua các điểm nhịp vàng!" % get_rhythm_action_text()
	prompt_label.visible = true
...
func _start_melody_round() -> void:
	...
	prompt_label.text = "Giai điệu ngũ cung của %s dưới đây đang khuyết một nốt nhạc. Nhấn nút Loa phát giai điệu rồi tìm nốt còn thiếu [?]:" % get_melody_instrument_name()
