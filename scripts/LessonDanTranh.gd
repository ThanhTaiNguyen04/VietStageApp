	...
	},
	{
		...
		"lessons": [
			...
			},
			{
				...
				"sheet": ["Đô2", "Rê2", "Fa2", "Sol2", "La2", "Sol2", "Fa2", "Rê2", "Đô2"]
			}
		]
	},
	{
...
func _create_lesson_path(lesson: Dictionary, index: int, lessons: Array, completed: Array) -> VBoxContainer:
	...
	if has_video:
		var video_button := _create_circle_button("Hướng dẫn", str(lesson["title"]), video_unlocked, video_completed)
		video_button.name = "VideoBtn"
		video_button.pressed.connect(_open_video.bind(lesson))
		row.add_child(video_button)

	var practice_button := _create_circle_button("Thực hành", str(lesson["title"]), practice_unlocked, practice_completed)
	practice_button.name = "PracticeBtn"
	practice_button.pressed.connect(_open_practice.bind(lesson))
...
func _draw_lesson_path() -> void:
	...
	for child in lessons_hbox.get_children():
		...
		if v_btn:
			var v_center: Vector2 = col.position + row.position + v_btn.position + v_btn.size / 2.0
			centers.append(v_center)
			node_unlocked.append(not v_btn.disabled)
		if p_btn:
			var p_center: Vector2 = col.position + row.position + p_btn.position + p_btn.size / 2.0
			centers.append(p_center)
			node_unlocked.append(not p_btn.disabled)
...
func _create_lesson_card(lesson: Dictionary) -> PanelContainer:
	...
	if str(lesson["video"]) != "":
		var video_btn := _create_action_button("Xem hướng dẫn", false)
		video_btn.pressed.connect(_open_video.bind(lesson))
		actions.add_child(video_btn)
	var practice_btn := _create_action_button("Bắt đầu thực hành", true)
	practice_btn.pressed.connect(_open_practice.bind(lesson))
...
func _open_video(lesson: Dictionary) -> void:
	if _has_dragged_significantly:
		return
	var lesson_number := int(lesson["number"])
	...
	VideoPlayer.custom_subtitles = [
		...
		{"start": 7.5, "end": 10.0, "text": "Hoàn thành video để chuyển sang phần thực hành trên đàn ảo."}
	]
	PracticeRoom.current_song_title = str(lesson["practice_title"])
	var typed_sheet: Array[String] = []
	typed_sheet.assign(lesson["sheet"])
