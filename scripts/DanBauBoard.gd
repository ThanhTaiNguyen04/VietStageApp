...
func _draw() -> void:
	...
	# Bevel wooden frames
	draw_rect(Rect2(0.0, 0.0, W, 12.0), Color(0.38, 0.18, 0.05)) # Top frame
	draw_rect(Rect2(0.0, H - 12.0, W, 12.0), Color(0.38, 0.18, 0.05)) # Bottom frame
	draw_rect(Rect2(0.0, 0.0, 16.0, H), Color(0.32, 0.15, 0.04)) # Left block
	draw_rect(Rect2(W - 16.0, 0.0, 16.0, H), Color(0.32, 0.15, 0.04)) # Right block
	
	# Border highlight lines
	draw_line(Vector2(0, 12), Vector2(W, 12), Color(0.55, 0.28, 0.10, 0.45), 1.5)
	draw_line(Vector2(0, H - 12), Vector2(W, H - 12), Color(0.18, 0.08, 0.02, 0.65), 1.5)
	draw_line(Vector2(16, 0), Vector2(16, H), Color(0.55, 0.28, 0.10, 0.45), 1.5)
	draw_line(Vector2(W - 16, 0), Vector2(W - 16, H), Color(0.18, 0.08, 0.02, 0.65), 1.5)
	
	# Wood grains for background table
	for i in range(12):
		var gy := 12.0 + (H - 24.0) * (float(i) / 11.0)
		draw_line(Vector2(16, gy), Vector2(W - 16, gy), Color(0.11, 0.05, 0.02, 0.18), 1.0)
	...
	# 1. Draw tapered body with wood grains and gold borders
	...
	var rod_control := Vector2(x_left - 35.0 + _bend_offset * 0.5, body_center_y - 50.0)
	...
	for k in range(steps):
		...
		draw_line(shadow_pts[k], shadow_pts[k+1], Color(0, 0, 0, 0.22), thickness, true)
		draw_circle(shadow_pts[k], thickness * 0.5, Color(0, 0, 0, 0.22))

	# Draw actual horn rod
	for k in range(steps):
		var t_mid := float(k) / float(steps)
		var thickness := lerpf(10.0, 3.0, t_mid)
		var horn_color := Color(0.06, 0.05, 0.05)
		draw_line(rod_pts[k], rod_pts[k+1], horn_color, thickness, true)
		draw_circle(rod_pts[k], thickness * 0.5, horn_color)
		# Specular highlights
		var specular_color := Color(0.35, 0.35, 0.35, 0.3)
		draw_line(rod_pts[k] - Vector2(0, thickness * 0.15), rod_pts[k+1] - Vector2(0, thickness * 0.15), specular_color, thickness * 0.3, true)

	# 5. Draw pear-shaped gourd (Bầu)
	...
	if _pluck_amp > 0.005:
		...
		for k in range(1, 30):
			...
			var osc := sin(ratio * PI) * sin(ratio * PI * 3.0 - _pluck_time * spd) * _pluck_amp * 8.0
	...
	var string_col := C_GOLD_LIGHT if _pluck_amp > 0.1 else Color(0.88, 0.90, 0.94, 0.95)
	if _is_bending:
		string_col = Color(0.98, 0.55, 0.15) # highlight bending
...
func _draw_tapered_body(x_left: float, x_right: float, y_center: float, h_left: float, h_right: float) -> void:
	...
	# Drop shadow
	var shadow_offset := Vector2(0, 8.0)
	draw_colored_polygon(PackedVector2Array([tl + shadow_offset, tr + shadow_offset, br + shadow_offset, bl + shadow_offset]), Color(0.04, 0.02, 0.01, 0.45))
	
	# Rich Mahogany/Rosewood wood base
	draw_colored_polygon(PackedVector2Array([tl, tr, br, bl]), Color(0.22, 0.09, 0.03))
	
	# Wood grain lines inside
	var grain_count := 12
	for i in range(grain_count):
		var f := float(i + 1) / float(grain_count + 1)
		...
		for j in range(steps + 1):
			...
			var wave := sin(gx * 0.04 + f * 12.0) * 1.5 + cos(gx * 0.08) * 0.8
			var gy := y_center - cur_h / 2.0 + cur_h * f + wave
			grain_pts.append(Vector2(gx, gy))
		
		var grain_color := Color(0.12, 0.05, 0.01, 0.28) if i % 2 == 0 else Color(0.32, 0.16, 0.06, 0.09)
		draw_polyline(grain_pts, grain_color, 1.4, true)

	# Inner gold border inlay
	var inset := 4.0
	...
	draw_polyline(PackedVector2Array([tl_in, tr_in, br_in, bl_in, tl_in]), Color(0.85, 0.68, 0.22, 0.85), 1.2, true)
	draw_polyline(PackedVector2Array([tl, tr, br, bl, tl]), Color(0.08, 0.04, 0.02, 0.95), 2.2, true) # Dark rim

func _draw_mop_scrolls(x_left: float, x_right: float, y_center: float, h_left: float, h_right: float) -> void:
	var mop_color1 := Color(0.72, 0.92, 0.88, 0.40) # Silver-cyan xà cừ
	var mop_color2 := Color(0.90, 0.78, 0.86, 0.40) # Silver-pink xà cừ
	var mop_gold   := Color(0.82, 0.68, 0.25, 0.55) # Gold inlay highlights
	...
	for s in range(sections):
		...
		var u_ctrl1 := Vector2(sec_x_start + step_x * 0.25, uy_start - 10.0)
		var u_ctrl2 := Vector2(sec_x_start + step_x * 0.75, uy_end + 10.0)
		_draw_bezier_vine(Vector2(sec_x_start, uy_start), u_ctrl1, u_ctrl2, Vector2(sec_x_end, uy_end), mop_color1, 1.0)
		
		# Lower vine
		var ly_start := y_center + h_start * 0.24
		var ly_end := y_center + h_end * 0.24
		var l_ctrl1 := Vector2(sec_x_start + step_x * 0.25, ly_start + 10.0)
		var l_ctrl2 := Vector2(sec_x_start + step_x * 0.75, ly_end - 10.0)
		_draw_bezier_vine(Vector2(sec_x_start, ly_start), l_ctrl1, l_ctrl2, Vector2(sec_x_end, ly_end), mop_color2, 1.0)
...
func _draw_shaded_gourd(pos: Vector2, direction: Vector2) -> void:
	var base_col := Color(0.78, 0.58, 0.12)
	var mid_col  := Color(0.90, 0.73, 0.22)
	var high_col := Color(0.98, 0.88, 0.45)
	
	var bulb_center := pos
	var neck_center := pos + direction * 8.0
	var tip_center := pos + direction * 14.0
	
	draw_circle(bulb_center + Vector2(0, 3), 13.0, Color(0, 0, 0, 0.25))
	draw_circle(neck_center + Vector2(0, 2), 9.0, Color(0, 0, 0, 0.25))
	
	draw_circle(bulb_center, 12.0, base_col)
	draw_circle(neck_center, 8.0, base_col)
	draw_circle(tip_center, 4.5, base_col)
	
	draw_circle(bulb_center - direction * 1.5, 9.5, mid_col)
	draw_circle(neck_center - direction * 1.0, 6.0, mid_col)
	
	draw_circle(bulb_center - direction * 3.0, 5.0, high_col)
	draw_circle(bulb_center - direction * 3.0 + Vector2(-1.5, -1.5), 2.0, Color.WHITE)
...
func _draw_brass_collar(pos: Vector2, angle: float) -> void:
	...
	draw_colored_polygon(rect_pts, Color(0.72, 0.55, 0.15))
	draw_polyline(rect_pts, Color(0.95, 0.82, 0.45), 1.0, true)

func _draw_peg(pos: Vector2) -> void:
	draw_rect(Rect2(pos.x - 3, pos.y - 10, 6, 20), Color(0.24, 0.12, 0.04))
...
func _draw_bend_gauge(center: Vector2, radius: float, cents: float) -> void:
	...
	draw_polyline(bg_pts, Color(1, 1, 1, 0.08), 3.5, true)
	...
	draw_line(tick_start, tick_end, Color(0.85, 0.72, 0.35, 0.5), 1.5)
	...
	if abs(cents) > 2.0:
		...
		draw_polyline(fill_pts, Color(color.r, color.g, color.b, 0.22), 7.0, true)
...
func _draw_cents_readout(font: Font, pos: Vector2, cents: float) -> void:
	...
	var color := Color(0.95, 0.82, 0.45)
	if cents > 5.0:
		color = Color(0.35, 0.95, 0.65)
	elif cents < -5.0:
		color = Color(0.98, 0.45, 0.35)
	...
	draw_rect(badge_rect, Color(0.08, 0.04, 0.02, 0.85), true)
	draw_rect(badge_rect, Color(color.r, color.g, color.b, 0.35), false, 1.0)
...
func _draw_ivory_node(pos: Vector2, idx: int, is_target: bool, is_hovered: bool, glow_alpha: float, pulse_phase: float, font: Font) -> void:
	var brass_col  := Color(0.72, 0.55, 0.15)
	var ivory_col  := Color(0.97, 0.95, 0.90)
	...
	if is_target:
		...
		var glow_r := 16.0 + pulse * 4.0
		var glow_col := Color(0.85, 0.68, 0.22, 0.15 + pulse * 0.15)
		draw_circle(pos, glow_r, glow_col)
		draw_arc(pos, glow_r, 0.0, TAU, 24, Color(0.85, 0.68, 0.22, 0.3 + pulse * 0.2), 1.0)
		
	if glow_alpha > 0.01:
		var pluck_r := 10.0 + glow_alpha * 20.0
		draw_circle(pos, pluck_r, Color(1, 0.95, 0.75, glow_alpha * 0.4))
		draw_arc(pos, pluck_r, 0.0, TAU, 24, Color(1, 0.90, 0.5, glow_alpha * 0.5), 1.5)
	...
	draw_circle(pos, 3.2, Color(0.24, 0.15, 0.05))
