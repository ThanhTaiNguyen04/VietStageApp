...
func _open_quiz() -> void:
	...
	for lesson: Dictionary in get_current_lessons():
		...
		if not lid.is_empty():
			ids.append(lid)
	LearningActivityContextScript.configure("dan_bau", ids, "res://scenes/LessonDanBau.tscn")
...
func _build_sidebar() -> void:
	...
	if btn_minigame:
		btn_minigame.pressed.connect(func() -> void:
			LearningActivityContextScript.configure("dan_bau", [SecureDataManager.active_lesson_id], "res://scenes/LessonDanBau.tscn")
			_fade_to_scene("res://scenes/LearningActivitiesScreen.tscn")
		)
