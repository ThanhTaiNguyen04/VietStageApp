...
func _ready() -> void:
	...
	_setup_fullscreen_video_practice("res://nvaore/danbau.ogv")
...
func _play_audio(idx: int) -> void:
	if _recording:
		# Muted per user request: only recognize pitch, do not play dan bau sound feedback when recording
		return
...
func _setup_fullscreen_video_practice(guide_path: String) -> void:
	...
	if notation_vbox:
		...
		if guide_path.ends_with(".png") or guide_path.ends_with(".jpg"):
			...
			img_rect.scale = Vector2(2.6, 2.6)
			...
			guide_player.scale = Vector2(2.6, 2.6)
